...
extern uint8 refercount[PHYSTOP/PGSIZE];
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    ...
    acquire(&reflock);
    refercount[PGROUNDUP((PTE2PA(*pte)))/PGSIZE]--;
    if(do_free && refercount[PGROUNDUP((PTE2PA(*pte)))/PGSIZE] < 1){
    // if(do_free){
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    *pte |= PTE_COW;
    ...
    acquire(&reflock);
    refercount[PGROUNDUP((uint64)pa)/PGSIZE]++;
    release(&reflock);
  }
  return 0;
